...
class ArchivedAccountsScreenViewModel(
    accountRepository: AccountRepository,
) : ViewModel() {

    private val log by lazyLogger("ArchivedAccountsScreenVM")
    private val _events: MutableSharedFlow<Event> = eventSharedFlow()
    val events = _events.asSharedFlow()

    val accountItemList: StateFlow<List<ViewAccountListItem>> =
        accountRepository
            .getAccountsFlow()
            .map { accounts ->
                accounts
                    .filter(Account::isArchived)
                    .sortedBy(Account::reversePosition)
                    .map(ViewAccountListItem::Account)
            }
            .stateIn(viewModelScope, SharingStarted.Lazily, emptyList())

    fun onAccountItemClicked(
